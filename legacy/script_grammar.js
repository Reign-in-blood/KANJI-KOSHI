...
async function getFilteredRows() {
  try {
    const response = await fetch('vocabulairen4.csv');
    const data = await response.text();

    // Divise les lignes du CSV et filtre celles contenant au moins 5 colonnes
...
// Fonction pour afficher un texte aléatoire
async function displayRandomKanji() {
  ...
  if (filteredRows.length > 0) {
    ...
    document.getElementById('grammar-container-1').textContent = currentRow[1]; // Niveau
    document.getElementById('grammar-container-2').textContent = '-'; // Réinitialise réponse
    document.getElementById('grammar-container-3').textContent = '-'; // Réinitialise réponse
    document.getElementById('grammar-container-4').textContent = currentRow[4]; // Français

    // Affiche la réponse après 4 secondes
    revealTimeout = setTimeout(() => {
      if (currentRow.length >= 5) {
        document.getElementById('grammar-container-2').textContent = currentRow[2]; // Colonne 3
        document.getElementById('grammar-container-3').textContent = currentRow[3]; // Colonne 4
      }
    }, 4000);
  } else {
...
function resetContainers() {
  document.getElementById('grammar-container-1').textContent = '-';
  document.getElementById('grammar-container-2').textContent = '-';
  document.getElementById('grammar-container-3').textContent = '-';
  document.getElementById('grammar-container-4').textContent = '-';
  currentRow = [];
}
